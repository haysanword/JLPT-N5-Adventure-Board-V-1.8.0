...
const Avatar: React.FC<AvatarProps> = ({ avatarId, className = 'w-12 h-12' }) => {
  // If avatarId is a base64 string, render an img tag
  if (avatarId && avatarId.startsWith('data:image')) {
    return (
      <div className={`rounded-full overflow-hidden bg-slate-200 ${className}`}>
        <img src={avatarId} alt="User Avatar" className="w-full h-full object-cover" />
      </div>
    );
  }
