...
import ExamTile from '../features/exams/ExamTile';
...
const Board: React.FC<BoardProps> = ({ lessons, exams, onSelectLesson, onSelectExam, onGenerateCertificate }) => {
  ...
  return (
    <div className="space-y-8">
      {levels.map(level => {
        ...
        const examForLevel = exams.find(e => e.level === level);
        const completedInLevel = lessonsInLevel.filter(l => completedLessons.includes(l.id));
        const areAllLessonsComplete = lessonsInLevel.length > 0 && lessonsInLevel.length === completedInLevel.length;
          ...
          <div key={level}>
            ...
            {examForLevel && (
              <div className="mt-6">
                <ExamTile 
                  exam={examForLevel}
                  unlocked={areAllLessonsComplete}
                  onClick={() => onSelectExam(examForLevel)}
                />
              </div>
            )}
          </div>
